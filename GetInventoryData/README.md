...
| テーブル名 | 用途 |
|-----------|------|
| `public.ne_inventory_data` | 在庫情報の保存先（最新データ / カレントテーブル） |
| `public.ne_inventory_history` | 在庫情報の変更履歴保存先（履歴データ / ヒストリーテーブル） |
...
各 RPC 関数では、指定のデータ項目に変化がある場合のみカレントテーブル（`ne_inventory_data`）の `更新日時` を更新し、その変更されたレコードを履歴テーブル（`ne_inventory_history`）に追記します。
...
## Supabaseデータ同期・履歴管理仕様

ネクストエンジンから取得した在庫データの同期および変更履歴の保持は、GASとSupabase（RPC関数）による役割分担のもとで動作しています。

### 1. GASとSupabase（RPC関数）の役割分担
* **GAS (Google Apps Script)**: ネクストエンジンAPIから最新データを取得・整形し、JSON形式でSupabaseのRPC関数（`upsert_ne_inventory_data` / `upsert_ne_stock_data`）へ渡す**データ転送層**を担います。
* **Supabase (RPC関数 / PostgreSQL)**: 渡されたJSONを展開し、データベース内部で差分判定・カレントテーブルの更新・ヒストリーテーブルへの履歴追記を一括で実行する**ビジネスロジック・データ保持層**を担います。

### 2. テーブル構成とタイムスタンプの定義

| テーブル | 役割 | 更新日時（`更新日時`） | 記録日時（`recorded_at`） |
| :--- | :--- | :--- | :--- |
| **`ne_inventory_data`**<br>(カレントテーブル) | 最新の在庫状態を保持 | 差分更新時にRPC関数内の `NOW()` で書き込み | - |
| **`ne_inventory_history`**<br>(イベントヒストリーテーブル) | 変更があった履歴のみを蓄積 | カレントテーブル更新時の `更新日時` をそのまま継承 | テーブル定義の `DEFAULT NOW()` により挿入時に自動採番 |

#### 2つのタイムスタンプの役割
* **更新日時 (`更新日時`)**: 在庫数やJANコードなどのステータスが実際に変更された時刻を表します。
* **記録日時 (`recorded_at`)**: 履歴テーブルに新しい変更ログ行が作成（`INSERT`）されたデータベース上の物理時刻を表します。
* ※通常運用時はほぼ同時刻となりますが、バッチ処理の遅延や再実行が発生した際に「データ変化の発生時刻」と「DB保存の時刻」のタイムラグを追跡するトレーサビリティを確保しています。

### 3. 差分判定（Diff検知）と更新ロジック
無駄なデータベース書き込みとストレージ圧迫を防ぐため、以下の手順で差分更新を行います。

1. **JSON展開**: 受信した `json_data` を `jsonb_to_recordset` で一時的なテーブル形式に展開します。
2. **差分検知（UPSERT）**: 「商品コード」をキーとし、主要項目（在庫数・引当数・フリー在庫数・欠品数・JANコード）または `is_active` フラグのいずれかに変更がある場合のみ、カレントテーブルの更新を実行します（`IS DISTINCT FROM` 判定）。
3. **履歴テーブルへの追記**: `RETURNING` 句によって実際にカレントテーブルで更新が行われた行（`upserted`）のみを抽出し、`ne_inventory_history` へ `INSERT` します。数値やJANコードに変更がない商品は更新がスキップされ、過去の「更新日時」が維持されます。
