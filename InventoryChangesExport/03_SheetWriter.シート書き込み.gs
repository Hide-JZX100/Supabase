 ...
 * シートが存在しない、またはヘッダーが不足している場合は、自動的にヘッダーを書き込む機能を備えています。
 */
 ...
 * 空文字、空行を排除して取得します。
 * シートが空の場合、自動的に1行目に「商品コード」ヘッダーを書き込みます。
 * 
...
function getItemCodesFromInputSheet_() {
  ...
  if (lastRow === 0) {
    ...
    console.log("入力シート '" + sheetName + "' にヘッダー「商品コード」を自動設定しました。");
    return [];
  }
 ...
 * 1行目のヘッダーが存在しない場合は、自動的にヘッダーを書き込みます。
 * 
